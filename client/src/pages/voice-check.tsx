...
const VoiceCheck: React.FC = () => {
  ...
  const analyzeVoice = async () => {
    if (transcript.trim()) {
      ...
      try {
        ...
        // Send transcript to backend for scam analysis
        ...
        const analysis = apiResult.analysis || apiResult;
        const finalTranscript = apiResult.transcript || transcript;
        
        setAnalysisResult({
          command: finalTranscript,
          action: 'analyze',
          risk_score: analysis.confidence || analysis.risk_score || 0,
          fraud_type: analysis.scam_type || 'Unknown',
          is_scam: analysis.is_scam || false,
        });
        
        setAdvancedAnalysisResults(analysis);
      ...
      // Fallback: send audio to backend for AssemblyAI transcription
      try {
        setIsProcessing(true);
        ...
        const finalTranscript = apiResult.transcript || '';
        ...
        console.log('Audio transcription and analysis completed:', apiResult);
  ...
  const processAudioFile = async () => {
    ...
    // Only allow mp3 for fraud analysis
    if (!uploadedFile.name.endsWith('.mp3')) {
      alert('Please select an mp3 file for fraud analysis.');
      return;
    }
    ...
    try {
      ...
      xhr.open('POST', '/api/ai/analyze-voice-file', true);
      xhr.send(formData);
      ...
      const { label, probability, transcript } = response;
      setTranscript(transcript);
      setShowTranscript(true);
      setAdvancedAnalysisResults(response);
      setAnalysisResult({
        command: transcript,
        action: 'analyze',
        risk_score: probability,
        fraud_type: label === 'fraud' ? 'Fraud' : 'Normal',
        is_scam: label === 'fraud',
      });
      console.log('Audio file fraud analysis completed:', response);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col p-4 max-w-md mx-auto">
      ...
      {/* Main content */}
      <Card className="bg-white dark:bg-[#181C24] border border-zinc-200 dark:border-zinc-700 shadow-lg transition-colors">
        <CardHeader>
        ...
        <CardContent className="flex flex-col items-center space-y-6">
          {activeTab === 'record' ? (
            <>
              ...
              <div className="w-full flex justify-center p-4 bg-slate-50 dark:bg-[#232837] rounded-lg transition-colors">
              ...
              {/* Transcript */}
              {!isRecording && (
                <div className="mt-4 p-2 bg-gray-100 dark:bg-[#232837] dark:text-gray-100 rounded transition-colors">
            ...
            <>
              <div className="w-full">
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 flex flex-col items-center justify-center bg-slate-50 dark:bg-[#232837] transition-colors">
          ...
          {analysisResult && (
            <div className="w-full mt-6">
              ...
              <Alert 
                ...
                className={analysisResult.is_scam 
                ...
                <AlertTitle className="flex items-center gap-2">
                  {analysisResult.is_scam ? (
                    ...
                    <>
                      ...
                      <span className="text-green-700 dark:text-green-200">No Scam Detected</span>
                    </>
                  )}
                </AlertTitle>
                <AlertDescription>
                  {analysisResult.is_scam
                    ...
                    : <span className="text-green-700 dark:text-green-200">No suspicious patterns were detected in this conversation.</span>}
                </AlertDescription>
              </Alert>
              ...
              {analysisResult.fraud_type && (
                <div className="mt-4">
                  ...
                  <Badge variant="outline" className="bg-amber-50 dark:bg-amber-900/60 dark:text-amber-100">
                    {analysisResult.fraud_type}
                  </Badge>
                </div>
              )}
